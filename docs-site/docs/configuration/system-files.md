...
| `BOOTCMDS.SYS` | Commands executed at bootup, after `XROUTER.CFG` and `IPROUTE.SYS`, for settings that have no `XROUTER.CFG` keyword (e.g. `GNET`, `PPP`). One command per line | n/a 
...
| `XENCAP.TXT` | Extended `ENCAP.TXT` allowing modes other than encap (`ipip`, `ipudp`). Experimental | n/a 
...
| `HTTPBAN.SYS` | Blocks malicious HTTP request URLs by signature and bans the sender IP. `ANYCASE` prefix for case-independent matching | n/a 
...
| File | Purpose | Notes |
| --- | --- | --- |
| `HTTP.ACL` | HTTP proxy egress control. Absent / no rules ⇒ all blocked (403) | n/a 
| `SOCKS.ACL` | SOCKS proxy egress control. Absent / no rules ⇒ all blocked | n/a 
...
| `PPPHOST.n` | PPP configuration, one file per modem port (`n` = port). Needed only for auto-answer PSTN | n/a 
| `*.SCR` | Dialler scripts | n/a 
...
| `LANGS.SYS` | Language-by-callsign selection at session start (`<callsign_mask> <lang_number>`; English 0, French 1, Spanish 2, German 3, Dutch 4). Unlisted callers get `DEFAULTLANG` | n/a 
