...
- **Primary sources mined:** the in-program manual mirrored on the OARC wiki
  ...
  packet Discord.
- **Checking (AI only, no human review):** every page was written by the model from a
  distilled research corpus, then put through three independent AI review passes
  (config/interfaces; networking/subsystems/commands; journeys/basics/admin plus a site-wide
  scope sweep). These flagged and corrected invented specifics, false-precision defaults,
  mis-citations and one altered quote. No human has fact-checked the content. The site builds
  clean under `mkdocs build --strict` (no broken internal links).
